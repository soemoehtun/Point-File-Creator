...
/** Turn one worksheet into a Dataset. */
function sheetToDataset(
  ...
  return { fileName, columns: headerRow, rows, sheetName, sheetNames: wb.SheetNames };
}
...
/** Read an Excel workbook once and keep it so sheets can be switched cheaply. */
export async function readWorkbook(file: File): Promise<XLSX.WorkBook> {
  const buf = await file.arrayBuffer();
  return XLSX.read(buf, { type: "array" });
}

/** Parse a specific sheet out of an already-read workbook. */
export function parseSheet(
  wb: XLSX.WorkBook,
  sheetName: string,
  fileName: string,
): Dataset {
  return sheetToDataset(wb, sheetName, fileName);
}

async function parseExcel(file: File): Promise<Dataset> {
  const wb = await readWorkbook(file);
  const first = wb.SheetNames[0];
  if (!first) throw new Error("This workbook has no worksheets.");
  return sheetToDataset(wb, first, file.name);
}
