...
export default function App() {
  ...
  const layerSwitcherRef = useRef<HTMLDivElement>(null);
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        if (isExcel(file)) {
          // Keep the workbook in memory so switching sheets is instant.
          const wb = await readWorkbook(file);
          if (!wb.SheetNames.length) throw new Error("This workbook has no worksheets.");
          workbookRef.current = wb;

          // Prefer the first sheet that actually contains rows.
          let chosen = wb.SheetNames[0];
          let ds: Dataset | null = null;
          for (const name of wb.SheetNames) {
            try {
              const candidate = parseSheet(wb, name, file.name);
          ...
          if (!ds) ds = parseSheet(wb, chosen, file.name);
          if (!ds.rows.length) throw new Error("The selected worksheet has no data rows.");

          loadDataset(ds);
          flash(
            `Loaded ${ds.rows.length.toLocaleString()} rows from “${ds.sheetName}”` +
              (wb.SheetNames.length > 1 ? ` · ${wb.SheetNames.length} sheets available` : ""),
          );
          return;
        }
  ...
  /** Switch to another worksheet in the already-loaded workbook. */
  const handleSheetChange = useCallback(
    (sheetName: string) => {
      const wb = workbookRef.current;
      if (!wb || !dataset) return;
      setError(null);
      try {
        const ds = parseSheet(wb, sheetName, dataset.fileName);
  ...
  // Close the basemap switcher when clicking outside it
  useEffect(() => {
    if (!layerOpen) return;
    const onDown = (e: MouseEvent) => {
      if (layerSwitcherRef.current && !layerSwitcherRef.current.contains(e.target as Node)) {
        setLayerOpen(false);
      }
    };
      ...
      <div className="workspace">
                      ...
                      <div style={{ marginTop: 18 }} className="source-summary">
                        ...
                        <div>
                          ...
                          <span>
                            ...
                            {dataset.rows.length.toLocaleString()} rows · {columns.length} columns
                          </span>
                        </div>
                      </div>

                      {dataset.sheetNames && dataset.sheetNames.length > 1 && (
                        <div className="sheet-picker">
                          <div className="mapping-label">
                            <label>Worksheet</label>
                            <span>{dataset.sheetNames.length} sheets</span>
                          </div>
                          <select
                          ...
                          <p className="field-help">
                            Switching sheets reloads the columns and clears the current mapping.
                          </p>
                        </div>
                      )}
        ...
        <main className="map-workspace">
          ...
          <div className="map-frame">
            ...
            <div className="layer-switcher" ref={layerSwitcherRef}>
              {!layerOpen ? (
                <button
                  className="layer-switcher-collapsed"
                  onClick={() => setLayerOpen(true)}
                  title="Change base map"
                  aria-label="Change base map"
                >
                  <img
                    src={(BASEMAPS.find((b) => b.id === mapLayer) ?? BASEMAPS[0]).thumb}
                    alt=""
                  />
                </button>
              ) : (
                <div className="layer-switcher-expanded">
                  {BASEMAPS.map((b) => (
                    <div
                      key={b.id}
                      className={`layer-tile ${mapLayer === b.id ? "active" : ""}`}
                      onClick={() => {
                        setMapLayer(b.id as MapLayer);
                        setLayerOpen(false);
                      }}
                    >
                      <div className="layer-tile-img" style={{ background: b.bg }}>
                        <img src={b.thumb} alt="" />
                      </div>
                      <div className="layer-tile-label">{b.label}</div>
                    </div>
                  ))}
                </div>
              )}
