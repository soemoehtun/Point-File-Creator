...
export default function MapView({
  ...
  const fitSigRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    // Cheap rolling hash of the visible id set — lets us detect a genuine
    // change of *which* points are shown (search / filter) versus a mere
    // restyle (colour, size), so we only re-zoom when the selection changes.
    let sig = points.length;
    ...
    // Zoom to whatever is currently visible whenever the id set changes,
    // e.g. after a search, a column filter, or the first draw.
    if (fitSigRef.current !== sig) {
      ...
      if (points.length === 1) {
        ...
        map.flyToBounds(
          ...
          { padding: [60, 60], maxZoom: 15, duration: first ? 0 : 0.5 },
        );
      }
    }
